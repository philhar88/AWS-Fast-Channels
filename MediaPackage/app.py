...
def generate_playback_urls(egress_endpoints, asset_id):
    """
    ...
    try:
        for egress_endpoint in egress_endpoints:
            ...
            if os.path.splitext(urlparse(egress_endpoint['Url']).path)[1] == '.mpd':
                ...
                url = urlunparse(url)
            playback_urls[os.environ['StackName']][asset_id].update(
                    {
                        packaging_configuration_id:url
                    }
                )
        logger.info('playback_urls: %s', json.dumps(playback_urls, default=list))
    except Exception as unknown_error:
        playback_urls = None
        raise unknown_error
...
def create_resource_id_from_mediaconvert_job_output(job_output):
    ...
    pattern = r'[^a-zA-Z0-9-]'
    try:
        asset_id = re.sub(pattern, '', os.path.splitext(job_output.path.lstrip("/"))[0])
    except Exception as unknown_error:
        asset_id = None
        raise Exception(unknown_error) from unknown_error
    return asset_id
...
def update_asset(asset):
    ...
    try:
        ...
        raise ValueError(not_found_exception.response['Error']['Message']) from not_found_exception
...
def lambda_handler(event, context):
    ...
    try:
        ...
        sleep(randint(10,30)/10) #randomize delay to minimize asset create collisions.
    ...
    #Tag resource with CloudFormation stack
    asset_tag = mediapackage.tag_resource(
    ...
    logger.info("Adding Tag: %s", json.dumps(asset_tag))
    ...
    playback_url_event = events.put_events(
        Entries=[
            {
                'Detail':json.dumps(
                    generate_playback_urls(asset['EgressEndpoints'],asset['Id'])
                ),
    ...
    logger.info("EventId Sent: %s", json.dumps(playback_url_event, default=str))
